...
import {GetpatientVitalsDetailsByUserIdApi} from '../../Services/appServices/General';
...
const NewCarousel = () => {
  ...
  useEffect(() => {
    ...
    GetpatientVitalsDetailsByUserIdApi(data, res => {
      setVitals(res);
    });
  }, []);
  ...
  return (
    ...
    <View>
      {vitals?.map(item => {
        return <Text style={{color: 'black'}}>{item.EntryDate}</Text>;
      })}
    </View>
  );
};
